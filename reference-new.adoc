...
This functionality enables NFS clients to write directly to the capacity tier in FabricPool. This feature has certain limitations and should be used with caution. For example, if you mix Cloud Write NFS traffic with non-cloud, regular ONTAP traffic, such as SnapMirror, the overall Recovery Point Objective (RPO) and Recovery Time Objective (RTO) in SnapMirror may be impacted.
