...
A few FabricPool performance improvements might impact your tiered data in Cloud Volumes ONTAP in the following scenarios: 
...
This functionality enables NFS clients to write directly to the capacity tier in FabricPool. This feature has certain limitations and should be used with caution. For example, if you mix Cloud Write NFS traffic with non-cloud, regular ONTAP traffic, such as SnapMirror, the overall RPO (Recovery Point Objective) and RTO (Recovery Time Objective) in SnapMirror may be impacted.
